...
from django.contrib.auth import REDIRECT_FIELD_NAME, login, get_user_model, authenticate
...
# class LRNAuthenticationBackend(ModelBackend):
#     def authenticate(self, request, username=None, password=None, lrn=None, **kwargs):
#         UserModel = get_user_model()
#         try:
#             user = UserModel.objects.get(username=username)
#         except UserModel.DoesNotExist:
#             print("invalid login")
#             print(username, password, lrn)
#             return None
#         else:
...
class LRNAuthenticationBackend(ModelBackend):
    def authenticate(self, request, lrn=None, username=None, password=None, **kwargs):
        UserModel = get_user_model()
        print(f"LRN: {lrn}")
        print(username)
        print(password)
        ...
        try:
            user = UserModel.objects.get(username=username, lrn=lrn)
            print(lrn)
            print(f'tried: {user}')
        except UserModel.DoesNotExist:
            return None
        ...
        if user.check_password(password):
            # Authenticate user with their LRN
            if user.lrn == kwargs.get('lrn'):
                return user
        return None


class LRNAuthenticationForm(AuthenticationForm):
    ...
    def clean(self):
        ...
        # First check if the user with the provided LRN exists
        try:
            user = UserModel.objects.get(lrn=lrn, username=username)
            print(user)
        except UserModel.DoesNotExist:
        ...
        # If user with provided LRN exists, authenticate them with username and password
        user = authenticate(lrn=lrn, username=username, password=password)
        ...
        # Check if the authenticated user has the same LRN as provided earlier
        if user.lrn != lrn:
            raise forms.ValidationError(
                "Invalid login credentials. Please try again.",
                code='invalid_login',
        ...
        if not user.is_active:
            raise forms.ValidationError(
                "Your account is not active yet. Wait for the administrator to verify your account.",
                code='invalid_login',
            )
        
        # If everything is valid, return the cleaned data
        return cleaned_data
